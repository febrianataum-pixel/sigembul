...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, config, setConfig, isSyncing }) => {
  ...
  return (
    <div className={`min-h-screen flex transition-colors duration-300 ${theme === 'dark' ? 'dark bg-slate-950' : 'bg-slate-50'}`}>
      <div className="flex w-full min-h-screen bg-inherit text-slate-900 dark:text-slate-100">
        {/* Sidebar Desktop */}
        ...
        <div className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarCollapsed ? 'md:ml-20' : 'md:ml-64'}`}>
          <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 h-14 md:h-16 flex items-center justify-between px-3 md:px-8 sticky top-0 z-30">
            <div className="flex items-center space-x-3">
              <button 
                onClick={() => setIsMobileMenuOpen(true)}
                className="md:hidden p-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-600 dark:text-slate-400"
              >
                <Menu size={20} />
              </button>
              <h2 className="text-[11px] md:text-lg font-black uppercase tracking-widest text-slate-800 dark:text-slate-100 truncate max-w-[120px] md:max-w-none">
            ...
            <div className="flex items-center space-x-2 md:space-x-4">
               ...
               <button 
                 onClick={toggleTheme}
                 className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-amber-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all shadow-sm"
               ...
               <div className="flex items-center space-x-2 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-full border border-slate-200 dark:border-slate-700">
                  <span className="text-[8px] md:text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-tight max-w-[60px] md:max-w-none truncate">
                    {config.operatorName || 'OPERATOR'}
                  </span>
                  <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-slate-900 dark:bg-blue-600 flex items-center justify-center text-white text-[8px] md:text-[10px] font-black uppercase shadow-inner">
